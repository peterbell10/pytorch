...
def should_reinplace_scatter(node: torch.fx.Node) -> bool:
    ...
    def is_realized(node):
        return (
            node.op in ("placeholder", "output") or node.target in needs_realized_inputs
        )

    # Input and output are realized anyway
    if is_realized(inp) and any(is_realized(user) for user in node.users):
        return True

    # Output is a copy back into the input, which forces both to be realized
    # as the output is a user of the input
    return any(
