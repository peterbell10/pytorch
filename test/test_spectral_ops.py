...
class TestFFT(TestCase):
    ...
    def test_istft_against_librosa(self, device, dtype):
        ...
        def librosa_istft(x, n_fft, hop_length, win_length, length, window, center):
            if window is None:
                window = np.ones(n_fft if win_length is None else win_length)
            else:
                window = window.cpu().numpy()
        ...
        def _test(size, n_fft, hop_length=None, win_length=None, win_sizes=None,
                  length=None, center=True, expected_error=None):
            ...
            result = x_stft.istft(n_fft, hop_length, win_length, window,
                                  length=length, center=center, normalized=True)
            ref_result = librosa_istft(x, n_fft, hop_length, win_length, window, length, center)
        ...
        for center in [True, False]:
            ...
            # spectral oversample
            _test(10, 7, 2, win_length=5, center=center)
            _test(4000, 1024, 512, win_length=100, center=center)
            _test(4000, 1024, 512, win_length=100, center=center, length=4000)
