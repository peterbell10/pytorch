...
template <typename scalar_t, typename VecLoadPolicy, typename ScalarLoadPolicy, typename StorePolicy>
void vectorized_inner_sum(
  ...
  for (int64_t j = 0; j < size1; ++j) {
    ...
    auto vec_acc = row_sum<vec_t, VecLoadPolicy>(row_in, vec_stride, vec_size);
    ...
    alignas(64) std::array<scalar_t, vec_t::size()> partials{};
...
template <typename scalar_t, typename LoadPolicy, typename StorePolicy>
void scalar_inner_sum(
  ...
  for (int64_t j = 0; j < size1; ++j) {
    ...
    auto ans = row_sum<scalar_t, LoadPolicy>(row_in, in_strides[0], size0);
...
void vectorized_outer_sum(
  ...
  for (; j + nrows * vec_t::size() <= size1; j += nrows * vec_t::size()) {
    ...
    auto sums = multi_row_sum<vec_t, nrows, VecLoadPolicy>(
  ...
  for (; j + vec_t::size() <= size1; j += vec_t::size()) {
    const auto *row_in = data[1] + j * scalar_stride;
    const vec_t sums = row_sum<vec_t, VecLoadPolicy>(
  ...
  for (; j < size1; ++j) {
    const auto *row_in = data[1] + j * scalar_stride;
    auto ans = row_sum<scalar_t, ScalarLoadPolicy>(row_in, inner_stride, size0);
    store<StorePolicy>(data[0], out_stride, j, ans);
  }
}
...
void scalar_outer_sum(
  ...
  for (; j + (nrows - 1) < size1; j += nrows) {
    const auto *row_in = data[1] + j * in_strides[1];
    auto sums = multi_row_sum<scalar_t, nrows, LoadPolicy>(
  ...
  for (; j < size1; ++j) {
    const auto *row_in = data[1] + j * in_strides[1];
    auto ans = row_sum<scalar_t, LoadPolicy>(
